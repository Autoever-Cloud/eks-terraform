...
resource "aws_iam_role" "eks_monitoring_ebs_csi_role" {
  name = "eks-monitoring-ebs-csi-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            ...
            "${replace(aws_iam_openid_connect_provider.monitoring_oidc.url, "https://", "")}:sub" = "system:serviceaccount:kube-system:ebs-csi-controller-sa"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy_attachment" "ebs_csi_policy_attachment_monitoring" {
