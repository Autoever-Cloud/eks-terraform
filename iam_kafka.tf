...
# 1. kafka EKS 클러스터(컨트롤 플레인)를 위한 IAM 역할
resource "aws_iam_role" "eks_kafka_cluster_role" {
  name = "eks-kafka-cluster-role" # 고유 이름
...
# 2. kafka 워커 노드(EC2 인스턴스)를 위한 IAM 역할
resource "aws_iam_role" "eks_kafka_node_role" {
  name = "eks-kafka-node-role" # 고유 이름
...
# 5. kafka EFS CSI 드라이버용 IAM 역할 (IRSA)
