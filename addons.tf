...
resource "aws_eks_addon" "datacenter_ebs_csi" {
  ...
  depends_on = [
    aws_iam_oidc_provider.datacenter_oidc,
...
resource "aws_eks_addon" "kafka_ebs_csi" {
  ...
  resolve_conflicts = "OVERWRITE"
  depends_on = [
    aws_iam_oidc_provider.kafka_oidc,
...
resource "aws_eks_addon" "monitoring_ebs_csi" {
  ...
  resolve_conflicts = "OVERWRITE"
  depends_on = [
    aws_iam_oidc_provider.monitoring_oidc,
